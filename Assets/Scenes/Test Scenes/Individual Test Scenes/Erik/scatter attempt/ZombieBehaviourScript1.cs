...
public class ZombieBehaviourScript : MonoBehaviour {
	...
	public ZombieState State {
		...
		set {
			...
			switch (value) {
				...
				case ZombieState.Manual:
					break;
				case ZombieState.Scatter:
					Vector3 delta = transform.position - Input.mousePosition;
					delta.Normalize();

					Vector2 velocity = rb.velocity;
					velocity.x = delta.x * ScatterSpeed;
					velocity.y = delta.y * ScatterSpeed;
	...
	void FixedUpdate() {
		...
		scatterTimer.Update(Time.deltaTime);
		...
		if (State != Player.HordeState) {
			if (State == ZombieState.Scatter && scatterTimer.IsRunning()) {
